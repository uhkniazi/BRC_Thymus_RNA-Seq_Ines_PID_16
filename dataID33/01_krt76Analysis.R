...
## this second filter cutoff is reduced from 0.8 to 0.5 as a lot of genes are
## missing expression due to the different nature of intestinal and thymus cells
table(ivProb < 0.5)
hist(ivProb)
mData = mData[!(ivProb < 0.5), ]
dim(mData)
...
fBatch = gsub('_\\d$', '', as.character(dfSample$title))
table(fBatch)
...
dfData = data.frame(Krt76=round(mData.norm[df$ENSEMBL,],0), Coef=fBatch)
